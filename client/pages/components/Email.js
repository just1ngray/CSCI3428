...
export default function Email(userAuthor) {
  ...
  async function getReq() {
    try {
      const req = await axios.get(`${serverURL}/api/email/${id}`, {
  ...
  const res = getReq();
  //sets email in localStorage.
  localStorage.setItem("emailData", res);
  ...
  if (userAuthor) {
      ...
      <div>
        ...
        <div>
          <TextBox
            ...
            setText={"test"}
          />
          <TextBox label="CC" rows="1" text={() => getCC() | ""} />
          <TextBox
            label="Subject"
            rows="1"
            text={() => getSubject() | "NO SUBJECT"}
          />
          <TextBox label="Body" rows="10" text={() => getBody() | ""} />
        </div>
        <div>
          <span>
            <CustomButton
    ...
    return (
      <div>
        ...
        <div>
          <TextBox label="From" rows="1" text={() => getFrom() | ""} />
          <TextBox label="CC" rows="1" text={() => getCC() | ""} />
          <TextBox
            label="Subject"
            rows="1"
